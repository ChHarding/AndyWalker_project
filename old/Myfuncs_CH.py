...
def call_api(latitude, longitude, altitude, units="M"):
    ...
    # CH: guard against bad/no response
...
def get_elevation(coord1, coord2, num_segments):
    ...
    # CH it's not necessary to init all list elements,
    # just start with [] and use append() to grow it 

    #Initialize a list to hold the terrain masking results for each point along the line
    #terrain_masked = [""] * len(responses) 
    terrain_masked = []
    ...
    for i, response in enumerate(responses):
        ...
        if response.Elevation() < alts[i]:
            #terrain_masked.append("No") 
            terrain_masked.append(False)
        else:
            #terrain_masked.append("Yes")
